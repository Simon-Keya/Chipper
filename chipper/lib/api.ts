...
export async function loginUser(username: string, password: string): Promise<{ token: string }> {
  if (isServer()) throw new Error("loginUser called on server");
...
export async function createCategory(data: FormData, token: string): Promise<Category> {
...
export async function updateCategory(id: number, data: FormData, token: string): Promise<Category> {
...
export async function updateOrderStatus(id: number, status: string, token: string): Promise<Order> {
...
export async function createOrder(
  orderData: { productId: number; quantity: number; customerDetails?: string; status?: string },
  token: string
): Promise<Order> {
...
export async function addReview(productId: number, reviewData: ReviewPayload, token: string): Promise<Review> {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ message: "Failed to add review" }));
