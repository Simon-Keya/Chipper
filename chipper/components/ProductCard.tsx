...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="group relative bg-white rounded-lg shadow-sm hover:shadow-md border border-gray-100 hover:border-emerald-200 transition-all duration-300 overflow-hidden flex flex-col h-full">
      {/* Hover Glow Effect - Subtle */}
      <div className="absolute inset-0 bg-gradient-to-br from-emerald-50/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none rounded-lg"></div>

      {/* Product Image Section */}
      <Link href={`/products/${product.id}`} className="relative block">
        {/* FIXED ASPECT RATIO: Use aspect-square for consistent square cards */}
        <div className="relative w-full aspect-square overflow-hidden bg-gray-100 rounded-t-lg">
          ...
          <div className="absolute top-3 right-3 z-10">
            {isInStock ? (
              <div className="flex items-center gap-1 px-2.5 py-1.5 bg-emerald-500 text-white text-xs font-bold rounded-full shadow-lg backdrop-blur-sm animate-fade-in">
                <div className="w-1.5 h-1.5 bg-white rounded-full animate-pulse"></div>
                In Stock
              </div>
            ) : (
              <div className="px-2.5 py-1.5 bg-gray-500 text-white text-xs font-bold rounded-full shadow-lg backdrop-blur-sm">
                Sold Out
              </div>
            )}
          </div>
      ...
      <div className="flex flex-col flex-grow p-2 sm:p-3 relative z-10">
        ...
        <div className="flex flex-col gap-2 mt-auto">
          ...
          <button
            ...
            disabled={!isInStock}
            className="group/whatsapp w-full flex items-center justify-center gap-2 px-3 py-2.5 text-xs sm:text-sm font-semibold text-white bg-[#25D366] hover:bg-[#22c35e] active:bg-[#1ea955] disabled:bg-gray-300 disabled:cursor-not-allowed rounded-lg shadow-md hover:shadow-lg transition-all duration-300 relative overflow-hidden"
            aria-label={`Order ${product.name} via WhatsApp`}
          >
            <span className="absolute inset-0 bg-white opacity-0 group-hover/whatsapp:opacity-20 transition-opacity"></span>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-3.5 w-3.5 sm:h-4 sm:w-4 fill-white relative z-10 group-hover/whatsapp:scale-110 transition-transform"
              viewBox="0 0 24 24"
            >
        ...
        {!isInStock && (
          <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center rounded-lg z-20">
            <div className="text-center p-4">
              ...
              <p className="font-bold text-gray-900 mb-1 text-sm">Out of Stock</p>
      ...
      <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-bl from-emerald-100/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-t-lg"></div>
    </div>
  );
}