...
/**
 * Combined:
 * - Uses client-side fetching logic (useEffect + fetchProducts/fetchCategories)
 * - Uses the full Chipper UI layout, animations, blobs, trust badges, and featured cards
 *
 * If you want a different brand or logo path, replace the `BRAND_NAME` / `LOGO_PATH` constants.
 */

const BRAND_NAME = 'Chipper';
const LOGO_PATH = '/logo.png'; // change if you have a different path
...
export default function HomePage() {
  ...
  // client-side fetch logic (keeps same behavior you asked for)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute top-40 right-20 w-72 h-72 bg-blue-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000" />
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-emerald-50 via-white to-blue-50">
        ...
        <div className="absolute inset-0">
          <div className="absolute inset-0 bg-[url('/patterns/dots.svg')] opacity-5 animate-pulse-slow" />
          <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-emerald-300 to-transparent animate-shimmer" />
        </div>
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex items-center justify-center gap-4 mb-6">
              <div className="w-12 h-12 relative">
                <Image src={LOGO_PATH} alt={`${BRAND_NAME} logo`} fill className="object-contain" />
              </div>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-100 text-emerald-700 rounded-full text-sm font-semibold shadow-sm hover:shadow-glow transition-all duration-300 animate-fade-in cursor-default group">
            ...
            {/* Animated Title */}
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold mb-4 leading-tight animate-fade-in animation-delay-200">
            ...
            <p className="text-lg md:text-xl text-gray-700 mb-8 max-w-2xl mx-auto leading-relaxed animate-slide-up animation-delay-400">
              Discover quality products for every need — electronics, fashion, home goods and more. Shop with confidence and order via WhatsApp.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center animate-slide-up animation-delay-600">
              <Link
                href="/products"
                className="group relative inline-flex items-center justify-center btn btn-lg bg-emerald-600 hover:bg-emerald-700 text-white border-0 shadow-lg hover:shadow-glow-lg transform hover:scale-105 transition-all duration-300 min-w-[200px] overflow-hidden px-6 py-3"
              >
                <span className="absolute inset-0 bg-gradient-to-r from-emerald-400 to-blue-500 opacity-0 group-hover:opacity-20 transition-opacity duration-300" />
              ...
              <a
                href="#categories"
                className="group inline-flex items-center justify-center btn btn-lg bg-white hover:bg-gray-50 text-gray-800 border-2 border-gray-300 hover:border-emerald-600 min-w-[200px] transition-all duration-300 shadow-sm hover:shadow-md relative overflow-hidden px-6 py-3"
              >
                <span className="absolute inset-0 bg-emerald-50 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <svg viewBox="0 0 1440 100" className="w-full h-auto fill-white">
            <path d="M0,50 C360,90 720,10 1440,50 L1440,100 L0,100 Z" />
          </svg>
        </div>
      </section>

      {/* Categories Section */}
      <section id="categories" className="container mx-auto px-4 py-12 bg-white relative">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {categories.map((category, index) => (
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-br from-emerald-400/20 to-blue-400/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <div className="absolute -top-10 -right-10 w-32 h-32 bg-emerald-100 rounded-full blur-2xl group-hover:bg-emerald-200 transition-colors duration-500 opacity-50" />
              ...
              <div className="absolute top-2 right-2 w-2 h-2 bg-emerald-400 rounded-full opacity-0 group-hover:opacity-100 group-hover:animate-ping" />
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-16 bg-gradient-to-b from-white to-gray-50 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {loading ? (
            ...
            Array.from({ length: 4 }).map((_, i) => (
              <div key={`skeleton-${i}`} className="bg-white rounded-2xl shadow-sm p-6 animate-pulse" />
            ))
          ) : (
                ...
                <div className="relative aspect-square overflow-hidden bg-gray-100">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center">
                    ...
                    >
                      ...
                      <span className="absolute inset-0 bg-emerald-100 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-center" />
                    </Link>
                  </div>

                  <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-bl from-emerald-400/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                </div>

                <div className="p-5 relative">
                  <div className="absolute inset-0 bg-gradient-to-t from-emerald-50/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-b-2xl" />
        ...
        <div className="text-center mt-12">
          <Link
            href="/products"
            className="group inline-flex items-center gap-2 btn btn-lg bg-white hover:bg-emerald-50 text-gray-800 border-2 border-gray-300 hover:border-emerald-600 shadow-md hover:shadow-xl transition-all duration-300 hover:scale-105 relative overflow-hidden px-6 py-3"
          >
            <span className="absolute inset-0 bg-gradient-to-r from-emerald-400 to-blue-400 opacity-0 group-hover:opacity-10 transition-opacity duration-300" />
      ...
      <section className="container mx-auto px-4 py-16 bg-white relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-100 rounded-full filter blur-3xl opacity-20 animate-blob" />
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-blue-100 rounded-full filter blur-3xl opacity-20 animate-blob animation-delay-2000" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10">
          {[
            ...
            { icon: 'whatsapp', title: 'WhatsApp Orders', desc: 'Easy ordering through WhatsApp', color: 'purple', gradient: 'from-purple-50 to-pink-50' },
            { icon: 'support', title: '24/7 Support', desc: 'We\'re here to help with your queries', color: 'amber', gradient: 'from-amber-50 to-orange-50' }
          ].map((item, idx) => (
            <div key={idx} className={`group text-center p-6 rounded-xl bg-gradient-to-br ${item.gradient} hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-3 hover:scale-105 cursor-default relative overflow-hidden`}>
              <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-emerald-400 to-blue-400 opacity-0 group-hover:opacity-20 transition-opacity duration-500" />
              ...
              <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-bl from-white/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            </div>
          ))}
        </div>
      </section>
